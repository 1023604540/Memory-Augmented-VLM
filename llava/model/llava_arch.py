...
class LlavaMetaForCausalLM(MultimodalOpsMixin, ABC):
    ...
    def encode_images(self, images):
        ...
        image_features = self.get_model().mm_projector(image_features).detach()
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None):
        ...
        if type(images) is list or images.ndim == 5:
            ...
            for image in images_list:
                ...
                rank_print(f"Sampling {sample_frames} frames from {num_frames} total frames.")
                # indices = torch.arange(num_frames)
                sampled_tensor = image[indices]  # [sample_frames, 196, 3584]
            ...
            for idx, image in enumerate(image_features):
                ...
                #     rank_print(f"Attention stats 2: {attn_stats[2]}")
        ...
        if self.training and drop_frame and dropout_frames:
            ...
            fake_memory = torch.zeros_like(image_features[0])  # [n, 3584]
            image_features_with_prompt = [
                torch.cat((memory_prompt_embeds, fake_memory, frame_prompt_embeds, image_features[1]), dim=0)
            ]
