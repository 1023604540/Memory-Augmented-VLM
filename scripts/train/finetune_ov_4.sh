...
RUN_NAME="llava-onevision-0.5b-qwen2_KIT_position_8tokens_catmemory_adapteron_mc"
...
srun --mpi=pmix --export=ALL,ACCELERATE_CPU_AFFINITY=0 \
  torchrun --nproc_per_node="${NUM_GPUS}" --nnodes="${NNODES}" --node_rank="${RANK}" --rdzv_backend=c10d \
    ...
    --mm_tunable_parts="larimar_model,recurrent_model,mm_language_model,mm_mlp_adapter" \
